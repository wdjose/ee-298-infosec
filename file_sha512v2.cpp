...
void get_hash(ull *M, ull *H) {
	...
	for (int i = 0; i < 80; i++) {
		ull S1 = right_rotate(w_temp[4], 14) ^ right_rotate(w_temp[4], 18) ^ right_rotate(w_temp[4], 41);
		ull ch = (w_temp[4] & w_temp[5]) ^ (~w_temp[4] & w_temp[6]);
		ull temp1 = w_temp[7] + S1 + ch + k[i] + w[i];
		ull S0 = right_rotate(w_temp[0], 28) ^ right_rotate(w_temp[0], 34) ^ right_rotate(w_temp[0], 39);
		ull maj = (w_temp[0] & w_temp[1]) ^ (w_temp[0] & w_temp[2]) ^ (w_temp[1] & w_temp[2]);
		ull temp2 = S0 + maj;

		for (int i = 7; i > 0; i--)
			w_temp[i] = w_temp[i - 1];
		w_temp[0] = temp1 + temp2;
		w_temp[4] += temp1;
	}
	for (int i = 0; i < 8; i++)
		H[i] += w_temp[i];
...
int main(int argc, char **argv) {
	...
	int max_buf_size = 1024 * 1024 * 16;	//in bytes
